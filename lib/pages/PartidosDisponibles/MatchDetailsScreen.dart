...
class _MatchDetailsScreenState extends State<MatchDetailsScreen> with SingleTickerProviderStateMixin {
  ...
  final ValueNotifier<List<Map<String, dynamic>>> _positionsNotifier = ValueNotifier<List<Map<String, dynamic>>>([]);
  ...
  void _setupPaymentListener() {
    ...
    _paymentSubscription = paymentStatusController.stream.listen((event) async {
      ...
      debugPrint('MatchDetailsScreen: Evento de pago recibido: $status, paymentId: $paymentId');
      ...
      switch (status) {
            ...
            if (mounted) {
              ...
              try {
                ...
                if (mounted) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('Pago procesado, error al actualizar vista: $e'),
        ...
        case PaymentStatus.pending:
          ...
          if (mounted && (paymentId == null || !processedPaymentIds.contains(paymentId))) {
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 16),
                Text('Error cargando equipos', style: TextStyle(color: Colors.red, fontSize: 16, fontWeight: FontWeight.bold)),
                Text('Intente nuevamente', style: TextStyle(color: Colors.grey[600], fontSize: 14)),
              ],
            ),
          );
        }
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 16),
                Text('No hay equipos disponibles', style: TextStyle(color: Colors.grey[600], fontSize: 16, fontWeight: FontWeight.bold)),
              ],
            ),
          );
        }
        ...
        return FutureBuilder<bool>(
          ...
          builder: (context, userInTeamSnapshot) {
            if (userInTeamSnapshot.connectionState == ConnectionState.waiting) {
              return Center(child: CircularProgressIndicator(color: Colors.blue, strokeWidth: 3));
            }

            return ListView.builder(
              itemCount: snapshot.data!.length,
              itemBuilder: (context, index) => _buildTeamSection(snapshot.data![index]),
            );
          },
        );
      },
  ...
  Widget _buildTeamSection(MatchTeam team) {
    ...
    final Map<String, TeamPlayer> playersByPosition = {
      for (var p in team.players) if (p.position != null) p.position!: p
    };
    ...
    return Card(
      ...
      child: Column(
        children: [
          ListTile(
            leading: CircleAvatar(backgroundColor: teamColor.withOpacity(0.2), child: Text(team.emoji ?? '⚽')),
            title: Text(team.name, style: TextStyle(color: teamColor, fontWeight: FontWeight.bold)),
            trailing: Container(
              padding: EdgeInsets.symmetric(horizontal: 12, vertical: 6),
              decoration: BoxDecoration(color: teamColor.withOpacity(0.1), borderRadius: BorderRadius.circular(15)),
              child: Text('${team.playerCount}/${team.maxPlayers}', style: TextStyle(color: Colors.black.withOpacity(0.8), fontWeight: FontWeight.bold, fontSize: 15)),
            ),
          ),
          Container(
            height: 150,
            child: ListView(
              ...
              children: [
                ...
                ...positions.map((pos) => Padding(
                      ...
                      child: _buildPlayerSlot(pos['name'], playersByPosition[pos['name']], teamColor, icon: pos['icon']),
                    )),
              ],
            ),
          ),
...
''';
      ...
      final result = await Share.shareWithResult(message, subject: '¡Únete a nuestro partido de fútbol!');
      if (result.status == ShareResultStatus.success) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('¡Partido compartido con éxito!'), backgroundColor: Colors.green));
      } else if (result.status == ShareResultStatus.dismissed) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Compartir cancelado'), backgroundColor: Colors.orange));
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error al compartir: $e'), backgroundColor: Colors.red));
    }
  }

  Widget _buildPlayerSlot(String position, TeamPlayer? player, Color teamColor, {String? icon}) {
    final bool isAvailable = player == null;
    String? imageUrl = player?.user?.profileImage != null ? 'https://proyect.aftconta.mx/storage/${player!.user!.profileImage}' : null;
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Container(
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: isAvailable ? Colors.grey.shade300 : teamColor, width: 2),
                ...
                child: CircleAvatar(
                  ...
                  child: isAvailable && icon != null ? Image.asset(icon, width: 24, height: 24, color: Colors.grey.shade400) : null,
                ),
              ),
            ],
          ),
          ...
          Text(
            ...
            style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: isAvailable ? Colors.grey.shade400 : Colors.black),
            maxLines: 1,
  ...
  Future<bool> _checkUserInTeam(List<MatchTeam> teams) async {
    ...
    return teams.any((team) => team.players.any((player) => player.user?.id == currentUserId));
  }
                  ...
                  children: [
                    InkWell(
                      ...
                      child: Container(
                        width: 60,
                        height: 60,
                        decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: teamColor, width: 2)),
  ...
  void _showLeaveTeamDialog(MatchTeam team) async {
    ...
    final bool isPredefTeam = team.name != "Equipo 1" && team.name != "Equipo 2";
    ...
    bool? confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(isTeamCaptain && isPredefTeam ? 'Retirar equipo' : 'Abandonar equipo', style: TextStyle(color: Colors.black)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
              isTeamCaptain && isPredefTeam ? '¿Estás seguro que deseas retirar a todo el equipo del partido?' : '¿Estás seguro que deseas abandonar el equipo?',
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Expanded(
                      child: Text('Esta acción retirará a todos los miembros del equipo.', style: TextStyle(color: Colors.red, fontStyle: FontStyle.italic)),
                    ),
                  ],
                ),
              ),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: _isLoading
                ...
                : Text(isTeamCaptain && isPredefTeam ? 'Retirar equipo' : 'Salir', style: TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
    ...
    if (confirmed == true && mounted) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isTeamCaptain && isPredefTeam ? 'El equipo ha sido retirado exitosamente' : 'Has abandonado el equipo exitosamente'),
            backgroundColor: Colors.green,
          ),
        );
      } catch (e) {
            ...
            children: [
              AlertDialog(
                title: Row(
                  ...
                  children: [
                    Text(showRules ? 'Normas del evento' : 'Unirse al partido', style: TextStyle(color: Colors.black)),
                    IconButton(
                  ...
                  child: Column(
                    ...
                    children: [
                      if (!showRules) ...[
                        Row(
                          children: [
                            Text('Unirse como: ', style: TextStyle(color: Colors.black)),
                            DropdownButton<bool>(
                              value: joinAsTeam,
                              items: [
                                DropdownMenuItem(value: false, child: Text('Individual', style: TextStyle(color: Colors.black))),
                                DropdownMenuItem(value: true, child: Text('Equipo', style: TextStyle(color: Colors.black))),
                              ],
                              onChanged: (value) {
                                setDialogState(() {
                                  joinAsTeam = value ?? false;
                                  if (!joinAsTeam) _selectedPredefinedTeam = null;
                                });
                              },
                            ),
                          ],
                        ...
                        if (joinAsTeam)
                              ...
                              if (teams.isEmpty) {
                                return Column(
                                  children: [
                                    ...
                                    SizedBox(height: 8),
                                    Text('No tienes equipos disponibles', style: TextStyle(fontSize: 16, color: Colors.grey[600])),
                                    SizedBox(height: 8),
                                    Text('Crea un equipo primero para poder inscribirlo', style: TextStyle(fontSize: 14, color: Colors.grey)),
                                  ],
                                );
                              }
                              return Column(
                                  ...
                                  if (_selectedPredefinedTeam != null)
                                    Padding(
                                      ...
                                      child: ElevatedButton(
                                        ...
                                        child: Text('Inscribir Equipo', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                                      ),
                                    ),
                                ],
                              );
                          ...
                          FutureBuilder<List<dynamic>>(
                            ...
                            builder: (context, snapshot) {
                              ...
                              if (snapshot.hasData && snapshot.data!.isNotEmpty) {
                                return Padding(
                                  padding: EdgeInsets.only(bottom: 8), // Corregido de 'custom' a 'bottom'
                                  child: Text(
                          ...
                          SizedBox(height: 8),
                          Text('Selecciona tu posición:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
                          SizedBox(height: 8),
                          ...availablePositions.map((position) {
                            ...
                            return Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(color: isOccupied ? Colors.grey.shade300 : Colors.blue.shade200, width: 1),
                              ),
                              child: ListTile(
                                enabled: !isOccupied && !_isLoading,
                                leading: Container(
                                  padding: EdgeInsets.all(8),
                                  decoration: BoxDecoration(color: isOccupied ? Colors.grey.shade200 : Colors.blue.shade50, borderRadius: BorderRadius.circular(8)),
                                  child: Image.asset(position['icon'], width: 24, height: 24, color: isOccupied || _isLoading ? Colors.grey.shade400 : Colors.blue.shade700),
                                ),
                                title: Text(position['name'], style: TextStyle(color: isOccupied || _isLoading ? Colors.grey.shade400 : Colors.black, fontWeight: FontWeight.bold)),
                                subtitle: isOccupied
                        ...
                        ListTile(
                          ...
                          title: Text('Normas del evento', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black)),
                        ),
                        CheckboxListTile(
                          value: normas1,
                          onChanged: (value) => setDialogState(() => normas1 = value ?? false),
                          title: Text('Estar 15 minutos antes del partido', style: TextStyle(color: Colors.black)),
                          controlAffinity: ListTileControlAffinity.leading,
                        ),
                        CheckboxListTile(
                          value: normas2,
                          onChanged: (value) => setDialogState(() => normas2 = value ?? false),
                          title: Text('Reembolso al monedero en caso de cancelación', style: TextStyle(color: Colors.black)),
                          controlAffinity: ListTileControlAffinity.leading,
                        ),
                        CheckboxListTile(
                          value: normas3,
                          onChanged: (value) => setDialogState(() => normas3 = value ?? false),
                          title: Text('Acepto las normas del evento', style: TextStyle(color: Colors.black)),
                          controlAffinity: ListTileControlAffinity.leading,
                        ),
                        SizedBox(height: 16),
                        SizedBox(
  ...
  Future<void> _leaveTeamAsGroup(MatchTeam team) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('El equipo ha sido retirado exitosamente'), backgroundColor: Colors.green));
    } catch (e) {
      setState(() => _isLoading = false);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error al retirar el equipo: $e'), backgroundColor: Colors.red));
    }
  }
  ...
  Future<void> _leaveTeam() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Has abandonado el equipo exitosamente'), backgroundColor: Colors.green));
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error al abandonar el equipo: $e'), backgroundColor: Colors.red));
    }
  }
Future<void> _handleJoinTeam(MatchTeam team, String? position,
    {required bool joinAsTeam, Equipo? predefinedTeam, bool useBono = false}) async {
  try {
    setState(() => _isLoading = true);

    if (joinAsTeam && predefinedTeam != null) {
      // Registrar el equipo predefinido primero
      final response = await MatchService().registerPredefinedTeamForMatch(widget.match.id, predefinedTeam.id, team.id);
      ...
      if (mounted) {
        ...
        await _showAssignPositionsDialog(registeredTeam, registeredTeam.players, predefinedTeam.id);
      }
      ...
      if (widget.match.price > 0 && !useBono) {
            ...
            builder: (context) => AlertDialog(
              content: Column(
                ...
                children: [
                  CircularProgressIndicator(),
                  SizedBox(height: 16),
                  Text('Procesando pago...', style: TextStyle(color: Colors.black)),
                ],
              ),
            ),
          );
        ...
        try {
          final result = await MatchService().processTeamJoinPayment(team.id, position, widget.match.price, widget.match.id, context);
          if (mounted) {
            Navigator.of(context).pop();
          }
          if (mounted) {
            final status = result['status'] as PaymentStatus? ?? PaymentStatus.failure;
            if (status == PaymentStatus.success || status == PaymentStatus.approved) {
              setState(() {
                _joinStatus = JoinTeamStatus.success;
                _loadTeams();
              });
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(useBono ? 'Bono utilizado, te has unido al equipo exitosamente' : 'Te has unido al equipo exitosamente'),
              backgroundColor: Colors.green,
            ),
          );
        }
  ...
  Future<void> _showAssignPositionsDialog(MatchTeam team, List<TeamPlayer> players, int predefinedTeamId) async {
    final availablePositions = PositionsConfig.getPositionsForFieldType(widget.match.gameType);
    ...
    return showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          title: Text('Asignar Posiciones', style: TextStyle(color: Colors.black)),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                Text('Asigna una posición a cada jugador del equipo', style: TextStyle(fontSize: 14, color: Colors.black)),
                SizedBox(height: 16),
                ...players.map((player) {
                  List<String> availablePositionsForPlayer = availablePositions
                      .map((pos) => pos['name'] as String)
                      .where((posName) => !takenPositions.contains(posName) || selectedPositions[player.id] == posName)
                      .toList();
                        ...
                        children: [
                          Row(
                            children: [
                              CircleAvatar(
                                radius: 20,
                                backgroundImage: player.user?.profileImage != null ? NetworkImage('https://proyect.aftconta.mx/storage/${player.user!.profileImage}') : null,
                                child: player.user?.profileImage == null ? Icon(Icons.person, color: Colors.grey) : null,
                              ),
                              SizedBox(width: 12),
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(player.user?.name ?? 'Jugador', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.black)),
                                    if (selectedPositions[player.id] != null) Text('Posición actual: ${selectedPositions[player.id]}', style: TextStyle(color: Colors.black, fontSize: 12)),
                                  ],
                                ),
                              ),
                            ],
                          ...
                          DropdownButtonFormField<String>(
                            decoration: InputDecoration(
                              ...
                              helperText: availablePositionsForPlayer.isEmpty ? 'No hay posiciones disponibles' : 'Selecciona una posición',
                              helperStyle: TextStyle(color: availablePositionsForPlayer.isEmpty ? Colors.red : Colors.grey[600]),
                            ),
                            value: selectedPositions[player.id],
                            items: availablePositionsForPlayer.map((posName) {
                              final positionData = availablePositions.firstWhere((pos) => pos['name'] == posName);
                              return DropdownMenuItem<String>(
                                value: posName,
                                child: Row(
                                  children: [
                                    Image.asset(positionData['icon'] as String, width: 24, height: 24, color: Colors.grey[700]),
                                    SizedBox(width: 8),
                                    Text(posName),
                                  ],
                                ),
                              ...
                              if (value != null) {
                                setDialogState(() {
                                  if (selectedPositions[player.id] != null) {
                                    takenPositions.remove(selectedPositions[player.id]);
                                  }
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                if (selectedPositions.length != players.length) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Asigna una posición a todos los jugadores'), backgroundColor: Colors.red));
                  return;
                }
                if (takenPositions.length != selectedPositions.length) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('No pueden haber posiciones duplicadas'), backgroundColor: Colors.red));
                  return;
                }
                try {
                  final response = await MatchService().registerPredefinedTeamForMatch(widget.match.id, predefinedTeamId, team.id);
                  debugPrint('Equipo inscrito: ${response['match_team_id']}');
                  final registeredTeamId = int.parse(response['match_team_id'].toString());

                  await Future.wait(selectedPositions.entries.map((entry) => 
                    MatchService().updatePlayerPosition(registeredTeamId, entry.key, entry.value)));

                  await MatchService().finalizeTeamRegistration(registeredTeamId);

                  Navigator.pop(context);
                  setState(() => _loadTeams());
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('Equipo inscrito y posiciones asignadas correctamente'),
                    backgroundColor: Colors.green,
                  ));
                } catch (e) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('Error al inscribir el equipo o asignar posiciones: $e'),
                    backgroundColor: Colors.red,
                  ));
                }
              },
              child: Text('Guardar Posiciones', style: TextStyle(color: Colors.white)),
            ),
          ],
        ),
      ),
